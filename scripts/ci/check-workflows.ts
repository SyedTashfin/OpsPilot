...
for (const job of requiredJobs) {
  ...
  if (job === "postgres-pgvector" && !block.includes("pnpm test:db")) {
    failures.push(`${job}: must run pnpm test:db for DB result validation`);
  }
}

const dbTestScript = packageJson.scripts?.["test:db"] ?? "";
for (const requiredFragment of [
  "--reporter=json",
  "--outputFile=.ci-results/db-migrations.json",
  "--outputFile=.ci-results/db-api-integration.json",
  "scripts/ci/validate-db-test-results.ts",
]) {
  if (!dbTestScript.includes(requiredFragment)) {
    failures.push(`test:db: missing ${requiredFragment}`);
  }
}
if (workflow.includes("grep -F") || workflow.includes("grep -E")) {
  failures.push("quality workflow must not use grep-based DB result validation");
}
