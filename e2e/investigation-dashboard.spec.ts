...
test("dashboard loads with real incidents and real health is not inferred from HTTP 200", async ({
  ...
  const recommendationIncidentRow = page
    .getByRole("row")
    .filter({ hasText: "recommendation-service" });
  await expect(recommendationIncidentRow).toContainText(
    "Recommendation latency spike after feature-store timeout deployment",
  );
  await expect(recommendationIncidentRow).toContainText("Recommendation Service");
...
test("authenticated deterministic investigation persists, reloads in history, and deep links", async ({
  ...
  await expect(page.locator(".root-cause")).toContainText(
