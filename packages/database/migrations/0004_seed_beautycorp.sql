...
SELECT id,
  ...
  'Symptoms: p95 latency above 1200ms, feature-store timeout errors, elevated retry count. First checks: compare latency with latest deployment, inspect feature_store_timeout_ms attributes, check cache hit ratio, and review downstream feature store health. Common root cause: a deployment reduces timeout budget or changes retry behavior, creating retry amplification. Safe response for V1 demo: rollback recommendation-service deployment rec-2026.06.1 or restore feature store timeout configuration after confirming logs and metrics.',
